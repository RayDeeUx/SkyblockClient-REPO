...
export default class QalcyoAutoresponse extends BotListener {
	...
	async exec(message) {
		...
		if (message.guild.id == '884212442664697937' || message.guild.id == '914828318409388102') {
			if (message.channel.parent.id == '901802497398693918' || message.channel.parent.id == '914830235382480936') {
				if (message.channel.type != 'GUILD_TEXT' && message.channel.type != 'GUILD_PUBLIC_THREAD') {
					return
				}
				if (message.content.startsWith('-') || message.content.startsWith('b!') || message.content.startsWith('t!') || message.content.startsWith('$') || message.content.startsWith('```'))
					return
