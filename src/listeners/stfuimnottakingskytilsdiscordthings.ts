    ...
    async exec(message) {
        ...
        notStolenFromSkytilsDiscordJson.forEach(async trigger => {
            ...
            if (contains && noAutorespond == false) {
                ...
                const filter = i => i.customId === 'autoresponseDeleteMessage' && i.user.id == message.author.id || i.customId === 'autoresponseDeleteMessage' && message.member.roles.cache.contains('780182606628782100')
